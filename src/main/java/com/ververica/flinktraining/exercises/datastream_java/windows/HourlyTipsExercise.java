...
public class HourlyTipsExercise extends ExerciseBase {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Tuple3<Long, Long, Float>> hourlyMax = fares
            ...
            .process(new ProcessWindowFunction<TaxiFare, Tuple3<Long, Long, Float>, Tuple, TimeWindow>() {
                @Override
                public void process(Tuple tuple, Context context, Iterable<TaxiFare> elements, Collector<Tuple3<Long, Long, Float>> out) throws Exception {
                    float tips = 0;
                    long winnerTimestamp = context.window().getEnd();
                    long winnerDriverId = -1L;
                    for (TaxiFare fare : elements) {
                        tips += fare.tip;
                        winnerDriverId = fare.driverId;
                    }
                    out.collect(new Tuple3<>(winnerTimestamp, winnerDriverId, tips));
                }
            })
